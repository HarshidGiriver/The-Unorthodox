...
from integration.services import calculate_portfolio_kpis
from integration.services import DebtRestructuringAgent
from integration.services import EmpatheticOutreachAgent
...
Cached anomaly indicators, generated financial proxies, and repayment proposal previews.
</p>
</div>
<div style="text-align: right;">
<span class="seal-badge">DEMO PORTFOLIO</span>
</div>
</div>""",
...
<div class="kpi-subtext">{kpis.get('anomaly_count', 0)} High Distress Accounts</div>
    ...
    with m4:
        st.markdown(
            f"""<div class="kpi-card">
<div class="kpi-label">Late-days / stress proxy</div>
...
<div class="kpi-subtext">{kpis.get('par_30_pct', 0):.1f}% of Active Book</div>
    ...
    with m5:
        st.markdown(
            f"""<div class="kpi-card">
<div class="kpi-label">Illustrative recovery scenario</div>
<div class="kpi-value" style="color: #0A523E;">₹{kpis.get('projected_npa_avoided', 0)/1e5:.1f} L</div>
<div class="kpi-subtext">Assumes 68% recovery; not a forecast</div>
</div>""",
            unsafe_allow_html=True,
        )
    ...
    with c1:
        ...
        fig_pie.update_layout(
            ...
            height=540,
            margin=dict(t=70, b=160, l=65, r=25),
            legend=dict(title_text="", orientation="v", x=0, xanchor="left", y=-0.32, yanchor="top", font=dict(size=11)),
        )
        for trace in fig_pie.data:
            trace.labels = [label.replace(" (Normal / Low Risk)", " - Low risk").replace(" (Moderate Stress)", " - Moderate").replace(" (High Anomaly / Severe Distress)", " - High anomaly") for label in trace.labels]
    ...
    with c2:
        ...
        fig_scatter.update_layout(
            ...
            xaxis=dict(gridcolor="#EADFCF", zerolinecolor="#C5A880", automargin=True,
                       title_standoff=18, tickangle=0, nticks=5, tickformat=".2f"),
            yaxis=dict(gridcolor="#EADFCF", zerolinecolor="#C5A880", automargin=True, title_standoff=12),
            legend=dict(title_text="", orientation="v", x=0, xanchor="left", y=-0.32, yanchor="top", font=dict(size=11)),
        )
        for trace in fig_scatter.data:
            trace.name = trace.name.replace(" (Normal / Low Risk)", " - Low risk").replace(" (Moderate Stress)", " - Moderate").replace(" (High Anomaly / Severe Distress)", " - High anomaly")
...
<span style="font-size: 0.85rem; font-weight: 700; color: #12211C;">Ranked by Unsupervised Stress Anomaly Score</span>
    ...
    if effective_query:
        mask = (
            filtered_df["customer_id"].astype(str).str.contains(effective_query, case=False, na=False, regex=False)
            | filtered_df["name"].astype(str).str.contains(effective_query, case=False, na=False, regex=False)
            | filtered_df["primary_drivers"].astype(str).str.contains(effective_query, case=False, na=False, regex=False)
        )
...
<h3 style="font-family: 'Cinzel', serif; color: #08201A; margin: 0 0 16px 0; font-weight: 800;">
 Case Review & Restructuring Action Center
    ...
    if filtered_df.empty:
        st.info("No accounts match these filters. Clear the search to review a borrower.")
        return
    triage_options = filtered_df["customer_id"].tolist()
    ...
    if selected_id:
        ...
        curr_emi = float(cust_record.get("current_emi", 14403.86))
        ...
        drivers = cust_record.get("primary_drivers", "Severe Outflow Burden, Above cohort mean, Liquidity Depletion")
        tier_label = cust_record.get("risk_tier", "Tier 3 (High Anomaly / Severe Distress)")

        # Benchmark behavioral metrics
        deal_surge_pct = ((deal_idx - float(df_portfolio["deal_reliance_index"].mean())) / float(df_portfolio["deal_reliance_index"].mean()) * 100) if deal_idx > float(df_portfolio["deal_reliance_index"].mean()) else 0.0
        ...
        with c_info:
            ...
            info_html = (
                ...
                f'</div>'
                f'<span class="badge-tier-{str(tier_label)[5:6]}">{tier_label}</span>'
                f'</div>'
                ...
                f'<div style="font-size: 0.72rem; color: #961515; text-transform: uppercase; font-weight: 800;">Above cohort mean</div>'
        ...
        with c_action:
            st.markdown("<h5 style='font-family: Cinzel, serif; color: #08201A; font-weight: 800;'>Restructuring Parameters</h5>", unsafe_allow_html=True)
            ...
            with st.expander("Preview Empathetic Outreach Notice (Draft — review required)"):
                ...
                st.info(
                    f"Configured contact window: {'Permissible (08:00 - 19:00)' if outreach['is_rbi_compliant_timing'] else 'Outside window; preview only, nothing is queued'}"
                )

            if st.button("Preview simulated underwriter approval"):
                st.info("Simulation only: proposal previewed. No approval is saved and no notification is sent.")
